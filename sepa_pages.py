...
def fetch_inst_day(ds):
    """單日三大法人買賣超金額（億）：外資(不含外資自營商) + 投信"""
    try:
        ...
        for row in d.get("data", []):
            nm = str(row[0]) if isinstance(row, list) and row else ""
            v = to_f(row[-1]) if isinstance(row, list) and row else None
            if v is None:
                continue
            if "外資及陸資" in nm and "自營" not in nm.replace("不含外資自營商", ""):
                out["foreign"] = round(v / 1e8, 1)
            elif nm.strip() == "投信" or ("投信" in nm and "自營" not in nm):
                out["trust"] = round(v / 1e8, 1)
        return out or None
    except Exception:
        pass
    return None
...
def main():
        ...
        if any("taiex" not in v for k, v in mh.items() if k[:6] == ym):
            ...
            for k, v in tm.items():
                if k in mh:
                    mh[k]["taiex"] = v
    # 三大法人買賣超（外資+投信）
    for k in list(fh.keys()):  # 舊格式（純數字）轉新格式
        if not isinstance(fh[k], dict):
            fh[k] = {"foreign": fh[k]}
    for ds in wdays:
        if ds not in fh or "trust" not in fh.get(ds, {}):
            v = fetch_inst_day(ds)
            if v:
                fh[ds] = v
            time.sleep(0.3)
    ...
    fkeys = sorted(k for k in fh if fh[k].get("foreign") is not None)[-120:]
    fseq = [{"date": k, "net": fh[k]["foreign"], "trust": fh[k].get("trust")} for k in fkeys]
    ...
    tseq = [x for x in fseq if x.get("trust") is not None]
    t_cum20 = round(sum(x["trust"] for x in tseq[-20:]), 0)
    t_cum_ytd = round(sum(x["trust"] for x in tseq if x["date"][:4] == str(TODAY.year)), 0)
    t_last = tseq[-1]["trust"] if tseq else None
...
def render(score, stage, bal, lo, hi, g20, tg20, div, mratio, seq,
           ...
           t_last=None, t_cum20=0, t_cum_ytd=0, tseq=None):
    ...
    tseq = tseq or []
    chart2b = svg_bars([x["trust"] for x in tseq[-60:]]) if tseq else ""
    cards2b = "".join([
        card("投信今日買賣超", f"{'+' if (t_last or 0)>=0 else ''}{t_last} 億" if t_last is not None else "—", "",
             "#ef4444" if (t_last or 0) >= 0 else "#22c55e"),
        card("投信近20日累計", f"{'+' if t_cum20>=0 else ''}{t_cum20:,.0f} 億", "",
             "#ef4444" if t_cum20 >= 0 else "#22c55e"),
        card("投信今年累計", f"{'+' if t_cum_ytd>=0 else ''}{t_cum_ytd:,.0f} 億", "",
             "#ef4444" if t_cum_ytd >= 0 else "#22c55e"),
    ])
    cards2 = "".join([
...
<div class="sec">
  ...
  {chart2}
  <div class="legend"><span>外資每日買賣超（近60日）｜外資=「外資及陸資(不含外資自營商)」，外資自營商金額極小不列入</span></div>
  <div style="height:12px"></div>
  <div class="grid" style="grid-template-columns:repeat(3,1fr)">{cards2b}</div>
  {chart2b}
  <div class="legend"><span>投信每日買賣超（近60日）— 內資主力，與外資對做時特別值得關注</span></div>
  <div style="height:12px"></div>
